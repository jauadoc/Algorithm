...
public class AnalyzeAlgorithm {
     ...
     *
     */
    public static void explainAnalyzeAlgorithm(){}
     ...
     *  1.求解规模为1的时间 Θ(1)<br>
     *  2.分解求解时间 <br>
     *      --将问题分解为a个子问题，每个子问题的规模为原问题的1/b，每个执行时间为函数T()，则每个子问题的规模为n/b，则子问题所需时间为T(n/b)，分解问题所需的时间为D(n)，合并问题解所需的时间为C(n)<br>
     ...
     * ----------T(n/n)=T(1)=c(分解到最底层的处理时间为假定规模1的处理时间c)=>cn+T(n/2)+T(n/2)=>cn+(cn/2+cn/2)+(T(n/4)+T(n/4)+T(n/4)+T(n/4))=>...=>cn+(cn/2*2)+...+(c)*n即可以推到出一颗递归树，递归树每级的代价都为cn
